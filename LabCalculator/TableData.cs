...
namespace LabCalculator
{
        public class TableData
        {
            ...
            public Dictionary<string, MyCell> cellList = new Dictionary<string, MyCell>(); // словник клітинок
        ...
        public void SetCellsInColumn(int nCol, DataGridView dataGridView1)
            ...
            public void ChangeCellsAndPointers(DataGridView dataGridView1, string cellName, string expression)
            {
                ...
                {
                    MessageBox.Show("З'явився цикл, змініть значення виразу");
                ...
                if (Value == "ERROR")
                {
                    MessageBox.Show("Виникла помилка в клітинці " + cellName);
                    cellList[cellName].Exp = "";
                    cellList[cellName].Value = "0";
                    dataGridView1.CurrentCell.Value = "0";
                    return;
            ...
            public bool RefreshCellAndPointers(MyCell cell, DataGridView dataGridView1)
            {
                ...
                if (Value == "ERROR")
                {
                    MessageBox.Show("Виникла помилка в клітинці " + cell.Name);
                    cell.Exp = "";
                    cell.Value = "0";
                    dataGridView1.CurrentCell.Value = "0";
        ...
        {
            List<MyCell> lastCol = new List<MyCell>();           //клітинки, які мають посилання в останньому стовпці
            ...
            if (lastCol.Count != 0 || notEmptyCells.Count != 0)
            {
                ...
                {
                    errorMessage = "ERROR є заповнені клітинки: видалення приведе до втрати даних \n";
                    errorMessage += string.Join(";", notEmptyCells.ToArray());
                }
                if (lastCol.Count != 0)
                {
                    errorMessage += "ERROR є посилання на інші клітинки в клітинках стовпця який ви хочете видалити:";
                    foreach (MyCell cell in lastCol)
                        errorMessage += string.Join(";", cell.Name);
                }
                errorMessage += "\nВи впевнені, що хочете видалити даний стопвець?";
        ...
        {
            List<MyCell> lastRow = new List<MyCell>();            //клітинки, які мають посилання в останньому рядку
            List<string> notEmptyCells = new List<string>();
            if (_rowNumber == 1)
                return false;
            ...
            if (lastRow.Count != 0 || notEmptyCells.Count != 0)
            {
                ...
                if (lastRow.Count != 0)
                {
                    errorMessage += "ERROR є посилання на інші клітинки в клітинках рядка який ви хочете видалити: ";
                ...
                errorMessage += "\nВи впевнені, що хочете видалити даний рядок?";
                System.Windows.Forms.DialogResult res = System.Windows.Forms.MessageBox.Show(errorMessage, "WARNING", System.Windows.Forms.MessageBoxButtons.YesNo, System.Windows.Forms.MessageBoxIcon.Warning);
                if (res == System.Windows.Forms.DialogResult.No)
                    return false;
            }
